...
Your regular routes tell anyone with database access where you sleep, where you work, which church or clinic or meeting you attend, and whose driveway your car spends the night in. The routes are the record.
...
Each step is documented above from primary sources. No step requires a warrant, a judge, or a council vote; the city's commitments on each are on the [[Promise Tracker]].

Step 1 by itself, a camera reading the plate in front of it, checked against a hot list and then discarded under a local retention policy with an audit trail someone reads, is a different object from steps 2 through 5, and it is not what this page warns about. Every rung above it is a separate decision: to retain, to pool, to join the national lookup, to add the live layer, to buy the person-lookup product. Each of those decisions has a default set by the vendor, and each can be declined, capped, or put under a public vote. The communities in [[Communities That Ended or Limited Contracts|Communities that ended or limited camera contracts]] did exactly that.

*Related: [[Who Gets Watched]] · [[Cameras in Public Housing|Cameras in Public Housing]] · [[What Is the RTIC]] · [[Getting Flock Out]] · [[Camera Network Misuse|Camera network misuse, documented cases]] · [[Asheville RTIC Claims and the Record|Asheville's RTIC claims and the record]]*
